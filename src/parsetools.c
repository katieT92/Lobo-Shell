...
void runPipes(struct Data_IDK shell_struct) {
    ...
    for (int i = 0; i < numCommands; i++) {                                          // For each command
        ...
        while (endNullSearchIdx <= shell_struct.num_words &&
               shell_struct.ArgV_S[endNullSearchIdx])         // Find next null for current command in "commands"
            endNullSearchIdx++;
        endNullSearchIdx++;
        char *command[(endNullSearchIdx - startNullSearchIdx) *
                      sizeof(char *)];         // Create "command" to hold current command in "commands"
        ...
        switch (pid = fork()) {                                                           // Parent forks a new child
            ...
            case 0:                                                                     // In Child process
                if (i == 0) {                                                            // First command in allCommands
                    dup2(pfd[1],
                         1);                                                    // Child stdout -> write end of pipe
                    if (close(pfd[0]) == -1 ||
                        close(pfd[1]) == -1)                     // Close both fd's, no longer need
                    ...
                    syserror("Could not exec simple command from first child.\n");
                               ...
                               1) {                                          // A middle command in allCommands
                    if (close(0) == -1)                                                 // Close fd for stdin
                        printf("Could not close stdin from child.\n");
                    dup2(oldFd,
                         0);                                                     // Point fd for stdin at previous read end of pipe
                    dup2(pfd[1],
                         1);                                                    // Point fd for stdout at write end of pipe
                    execvp(command[0], command);
                    if (close(pfd[0]) == -1 || close(pfd[1]) == -1 ||
                        close(oldFd))     // Close fd's for stdin and stdout, no longer need
                        syserror("Could not close pfd[0] of pfd[1] or oldFd from child.\n");
                } else {                                                                    // Last command in allCommands
                    dup2(oldFd, 0);
                    if (close(
                            oldFd))                                                    // Close fd for oldFd from previous pipe
                ...
                if (i == 0) {                                                            // First command
                    ...
                    if (close(pfd[1]) == -1)                                            // Close write end of pipe
                        syserror("Could not close pfd[1] from parent.\n");
                               ...
                               1) {                                          // A middle command in allCommands
                    if (close(oldFd) == -1 ||
                        close(pfd[1]) == -1)                      // Close end of previous pipe oldFd points at
                        syserror("Could not close oldFd or pfd[1] from parent.\n");
                    oldFd = pfd[0];                                                     // Point oldFd at read end of pipe
                } else {
                    // Last command in allCommands
                    if (close(oldFd) ==
                        -1)                                             // Close end of previous pipe that oldFd points at
